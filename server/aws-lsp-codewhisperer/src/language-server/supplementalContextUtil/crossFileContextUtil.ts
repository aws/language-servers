...
export async function fetchSupplementalContextForSrc(
    ...
    const relevantCrossFilePaths = await getCrossFileCandidates(editor, document)
 ...
 */
export async function getCrossFileCandidates(editor: vscode.TextEditor, document: TextDocument): Promise<string[]> {
     ...
     */
    const unsortedCandidates = await getOpenFilesInWindow(async (candidateFile: string) => {
        return (
