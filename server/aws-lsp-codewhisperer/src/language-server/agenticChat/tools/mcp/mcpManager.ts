...
export class McpManager {
    ...
    public async removeServer(serverName: string): Promise<void> {
        ...
        const persona = this.mcpServerPermissions.get(serverName)
        ...
        if (persona && persona.__configPath__) {
            await this.mutatePersonaFile(persona.__configPath__, p => p.removeServer(serverName))
        }
    ...
    /**
     * Safely read-modify-write the persona YAML, then refresh in-memory permission map.
     * @private
     */
