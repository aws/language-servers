...
describe('removeAllServers()', () => {
    let saveAgentConfigStub: sinon.SinonStub
    let loadAgentConfigStub: sinon.SinonStub

    beforeEach(() => {
        sinon.restore()
        saveAgentConfigStub = sinon.stub(mcpUtils, 'saveAgentConfig').resolves()
        loadAgentConfigStub = sinon.stub(mcpUtils, 'loadAgentConfig')
    })
    ...
    it('closes all clients and clears all state', async () => {
        const cfg1: MCPServerConfig = {
            command: 'server1',
            args: [],
            env: {},
        ...
        loadAgentConfigStub.resolves({
            ...
            agentConfig: {
                ...
                version: '1.0.0',
                description: 'Test agent',
                mcpServers: { server1: cfg1, server2: cfg2 },
                tools: ['@server1', '@server2', 'fsWrite'],
                allowedTools: ['@server1/tool1', 'fsWrite'],
                toolsSettings: {},
                includedFiles: [],
                resources: [],
            },
        ...
        const mgr = await McpManager.init(['config1.json', 'config2.json'], features)

        // Add some clients and tools to simulate active servers
        const client1 = new Client({ name: 'client1', version: '1.0.0' })
        const client2 = new Client({ name: 'client2', version: '1.0.0' })
        const closeStub1 = sinon.stub(client1, 'close').resolves()
        const closeStub2 = sinon.stub(client2, 'close').resolves()

        ;(mgr as any).clients.set('server1', client1)
        ;(mgr as any).clients.set('server2', client2)
        ;(mgr as any).mcpTools.push(
            { serverName: 'server1', toolName: 'tool1', description: 'desc1', inputSchema: {} },
            { serverName: 'server2', toolName: 'tool2', description: 'desc2', inputSchema: {} }
        )

        // Mock loadAgentConfig for individual config files - reset and reconfigure
        ...
        loadAgentConfigStub
            ...
            .resolves({
                agentConfig: {
                    ...
                    version: '1.0.0',
                    description: 'Test agent',
                    mcpServers: { server1: cfg1 },
                    tools: ['@server1', 'fsWrite'],
                    allowedTools: ['@server1/tool1', 'fsWrite'],
                    toolsSettings: {},
                    includedFiles: [],
                    resources: [],
            ...
            .resolves({
                agentConfig: {
                    ...
                    version: '1.0.0',
                    description: 'Test agent',
                    mcpServers: { server2: cfg2 },
                    tools: ['@server2', 'fsWrite'],
                    allowedTools: ['fsWrite'],
                    toolsSettings: {},
                    includedFiles: [],
                    resources: [],
                },
        ...
        expect((mgr as any).clients.size).to.equal(0)
        expect((mgr as any).mcpTools).to.be.empty
        expect((mgr as any).mcpServerStates.size).to.equal(0)
        expect((mgr as any).mcpServers.size).to.equal(0)
        expect((mgr as any).serverNameMapping.size).to.equal(0)
        expect((mgr as any).mcpServerPermissions.size).to.equal(0)

        // Verify agent config was cleared of MCP-related entries
        expect((mgr as any).agentConfig.mcpServers).to.deep.equal({})
        expect((mgr as any).agentConfig.tools).to.not.include('@server1')
        expect((mgr as any).agentConfig.tools).to.not.include('@server2')
        expect((mgr as any).agentConfig.allowedTools).to.not.include('@server1/tool1')

        // Verify built-in tools are preserved
        expect((mgr as any).agentConfig.tools).to.include('fsWrite')
        expect((mgr as any).agentConfig.allowedTools).to.include('fsWrite')

        // Verify saveAgentConfig was called for each agent path
        expect(saveAgentConfigStub.callCount).to.equal(2)
    })

    it('handles client close errors gracefully', async () => {
        loadAgentConfigStub.resolves({
            servers: new Map([
                ['server1', { command: 'cmd', args: [], env: {}, timeout: 0, __configPath__: 'config.json' }],
            ]),
            serverNameMapping: new Map(),
            errors: new Map(),
            agentConfig: {
        ...
        const mgr = await McpManager.init(['config.json'], features)

        // Add a client that will throw an error when closed
        const client = new Client({ name: 'client', version: '1.0.0' })
        sinon.stub(client, 'close').rejects(new Error('Close failed'))
        ;(mgr as any).clients.set('server1', client)

        // Reset and configure stub for this test
        loadAgentConfigStub.reset()
        loadAgentConfigStub
            .onFirstCall()
            .resolves({
        ...
        // Should not throw despite client close error
        await mgr.removeAllServers()

        // Verify error was logged
        expect(errorSpy.called).to.be.true
        // Create a safe check function that handles empty arrays
        const hasErrorMessage = (call: sinon.SinonSpyCall) => {
            if (!call.args || call.args.length === 0) {
                return false
            }
            const message = call.args[0]
            return typeof message === 'string' && message.includes('MCP: error closing client server1')
        }
        const errorCall = errorSpy.getCalls().find(hasErrorMessage)
    ...
    it('handles saveAgentConfig errors gracefully', async () => {
        // Initialize with a server so there's something to remove
        loadAgentConfigStub.resolves({
            servers: new Map([
                ['server1', { command: 'cmd', args: [], env: {}, timeout: 0, __configPath__: 'config.json' }],
            ]),
            serverNameMapping: new Map([['server1', 'server1']]),
            errors: new Map(),
            agentConfig: {
                name: 'test-agent',
                version: '1.0.0',
        ...
        // Set up agentPaths to ensure the loop in removeAllServers runs
        ;(mgr as any).agentPaths = ['config.json']

        // Reset and configure stub to fail on loadAgentConfig call
        loadAgentConfigStub.reset()
        loadAgentConfigStub.rejects(new Error('Load failed'))

        const warnSpy = sinon.spy(fakeLogging, 'warn')

        // Should not throw despite loadAgentConfig error
        await mgr.removeAllServers()
