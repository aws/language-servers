...
${workspaceFolder}/subfolder/file3.ts:1:const searchable = "found in subfolder";`,
    ...
    it('requires acceptance if the path is outside the workspace', async () => {
        const grepSearch = new GrepSearch(features)
        const result = await grepSearch.requiresAcceptance({ query: 'test', path: '/outside/workspace' })
        assert.equal(result.requiresAcceptance, true, 'Should require acceptance for path outside workspace')
    })

    it('does not require acceptance if the path is inside the workspace', async () => {
        const grepSearch = new GrepSearch(features)
        const result = await grepSearch.requiresAcceptance({ query: 'test', path: workspaceFolder })
        assert.equal(result.requiresAcceptance, false, 'Should not require acceptance for path inside workspace')
    })
