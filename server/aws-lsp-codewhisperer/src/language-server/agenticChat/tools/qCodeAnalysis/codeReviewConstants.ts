...
export const CODE_REVIEW_TOOL_DESCRIPTION = [
    'CodeReview tool is the MANDATORY and PRIMARY tool for ALL code analysis, finding code issues and code review. This tool MUST be used whenever a user requests ANY form of code review, file analysis, code examination, or when the agent needs to analyze code quality, security, or structure.',
    ...
    "- CRITICAL: After using this tool, do not attempt to provide additional manual code review or analysis that might contradict or duplicate the tool's findings.",
].join('\n')
