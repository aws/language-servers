    ...
    public getSpec() {
        return {
            ...
            description:
                ...
                '## Overview\n' +
                'This tool replaces sections of content in an existing file using `oldStr`/`newStr` blocks that define exact changes to specific parts of the file. You MUST ALWAYS determine the absolute path of the file to be modified and generate it before using this tool. You MUST ALWAYS group as many changes as you can by populating the diffs array with multiple `oldStr`/`newStr` pairs, DO NOT be overly cautious and methodical by making one change at a time on the same file.\n\n' +
                '## When to use\n' +
