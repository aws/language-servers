...
export class CsharpDependencyGraph extends DependencyGraph {
    ...
    private async generateFilePath(modulePath: string, dirPath: string) {
        const filePath = path.join(dirPath, modulePath + DependencyGraphConstants.csharpExt)
        return (await this.workspace.fs.exists(filePath)) ? filePath : ''
    }

    //For Generating File Paths
    private async generateFilePaths(modulePaths: string[], dirPaths: string[]) {
        const filePathsPromises = modulePaths.flatMap(async modulePath => {
            const filePaths = await Promise.all(
                dirPaths.map(async dirPath => await this.generateFilePath(modulePath, dirPath))
            )
            return filePaths.filter(filePath => filePath !== '') // Filtering out empty strings
        })

        const filePathsArrays = await Promise.all(filePathsPromises)
        return filePathsArrays.flat() // Flatten the array of arrays into a single array
    }
    ...
    async parseImport(importStr: string, dirPaths: string[]) {
        ...
        const dependencies = await this.generateFilePaths(modulePaths, dirPaths)
        return Promise.resolve(dependencies)
    }

    override updateSysPaths(uri: Uri) {
        this.getDirPaths(uri).forEach(dirPath => {
            this._sysPaths.add(dirPath)
        })
    }

    override async getDependencies(uri: Uri, imports: string[]) {
        const dependencies: string[] = []

        for (const importStr of imports) {
            this.updateSysPaths(uri)
            const importString = importStr.replace(';', '')
            const findings = await this.parseImport(importString, Array.from(this._sysPaths.values()))
            const validSourceFiles = findings.filter(finding => !this._pickedSourceFiles.has(finding))

            for (const file of validSourceFiles) {
                if (
    ...
    override async searchDependency(uri: string): Promise<Set<string>> {
        const filePath = uri
        const q: string[] = []
        q.push(filePath)
        while (q.length > 0) {
            ...
            while (count > 0) {
                ...
                const content: string = await this.workspace.fs.readFile(uri.fsPath)
    ...
    override async traverseDir(dirPath: string) {
        ...
        for (const file of files) {
            ...
            if (file.name.charAt(0) === '.' || !(await this.workspace.fs.exists(absPath))) {
                continue
            }
            ...
            if (file.isDirectory()) {
                ...
                if (
                    ...
                    !this.willReachSizeLimit(this._totalSize, (await this.workspace.fs.getFileSize(absPath)).size) &&
    ...
    override async generateTruncation(uri: string): Promise<Truncation> {
        try {
    ...
    */
    ...
    /* It is used for fetching cross-file context. 
    */
    override async getSamePackageFiles(uri: Uri, projectPath: string): Promise<string[]> {
        const fileList: string[] = []
        const packagePath = path.dirname(uri.fsPath)
        const files = await this.workspace.fs.readdir(packagePath)
        files.forEach(file => {
