...
export async function deviceCodeFlow(params: SsoFlowParams): Promise<SSOToken> {
    ...
    if (!authorization.verificationUriComplete || !authorization.userCode || !authorization.deviceCode) {
        throw new AwsError('Device authorization is invalid.', AwsErrorCodes.E_INVALID_SSO_CLIENT)
    }
    ...
    try {
        ...
        params.handlers.showProgress({ state: GetSsoTokenProgressState.Complete })
    }
...
function pollAuthorization(
    ...
    interval: number = backoffDelayMs
) {
    ...
    async function poll() {
        while (true) {
            if (expiresAt.getTime() - Date.now() <= interval) {
            ...
            await asyncCallWithTimeout(cancellationPromise, interval, '').catch(err => {
...
async function showOnClient(handlers: SsoFlowParams['handlers'], authorization: StartDeviceAuthorizationCommandOutput) {
    ...
    const response = await asyncCallWithTimeout(
        ...
        responseTimeout,
        `Timed out waiting for client message response for device code with interval ${responseTimeout}`
    ).catch(err => {
        if (err instanceof AsyncTimeoutError) {
