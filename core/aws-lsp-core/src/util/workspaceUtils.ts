...
export function getWorkspaceFolders(lsp: Features['lsp']) {
    const initParams = lsp.getClientInitializeParams()
    return initParams?.workspaceFolders?.map(({ uri }) => URI.parse(uri).fsPath) ?? []
}
