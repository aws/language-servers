...
import { Connector } from '../ideConnector'
...
export class QuickActionHandler {
    ...
    private handleGumbyCommand(tabID: string, eventId: string | undefined) {
        ...
        // NOTE: The below code means the following: if the current tab was actually used (not 'unknown' or welcome'),
        // create a new tab and later convert it into transform.
        // This will not work with migration to Flare, we'll need to change it for all agents
        // or find a way to mirror all tabs created in chat-client to 'tabsStorage'.
        // Currently, chat-client knows nothing about 'tabsStorage'.
        // Alternative way to achieve the same is below >>
        //
        // const currentTabType = this.tabsStorage.getTab(affectedTabId)?.type
        // if (currentTabType !== 'unknown' && currentTabType !== 'welcome') {
        //     affectedTabId = this.mynahUI.updateStore('', {
